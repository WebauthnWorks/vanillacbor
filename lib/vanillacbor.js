...
(function(){
    ...
    let readBE81632 = (buffer) => {
        if(typeof Buffer === 'undefined')
            buffer = enforceBigEndian(enforceUint8Array(buffer));

        if(buffer.length !== 1 && buffer.length !== 2 && buffer.length !== 4)
            throw new Error('Only 2/4/16 byte buffers are allowed!');

        if(buffer.length === 1)
            return new Uint8Array(Array.from(buffer))[0]
        else if(buffer.length === 2)
            return new Uint16Array(Array.from(buffer))[0]
        else
            return new Uint32Array(Array.from(buffer))[0]
    }
